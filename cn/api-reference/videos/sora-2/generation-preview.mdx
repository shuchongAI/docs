---
title: "Sora2 Preview 视频生成"
description: "
- 异步处理模式，返回任务ID用于后续查询

- 支持文本转视频、图生视频等多种生成模式

- 生成的视频链接，有效期为72小时，请尽快保存
"
...
payload <- list(
  ...
  duration = 8,
  aspect_ratio = "landscape",
  image_urls = c("https://example.com/reference-image.jpg")
)
...
<ParamField body="model" type="string" default="sora-2-preview" required>
  ...
  示例：`"sora-2-preview"` 或 `"sora-2-pro-preview"`
</ParamField>

<ParamField body="prompt" type="string" required>
  视频生成的文本描述

  详细描述您想要生成的视频内容，包括场景、动作、氛围等。

  示例：`"A majestic waterfall cascading down rocky cliffs, creating a beautiful rainbow in the mist"`
</ParamField>
...
<ParamField body="aspect_ratio" type="string" default="landscape">
  ...
  支持的格式：
  - `landscape` 或 `16:9` - 横屏 (16:9)
  - `portrait` 或 `9:16` - 竖屏 (9:16)
...
<ParamField body="size" type="string">
  ...
  此参数与 `aspect_ratio` 功能相同，二选一即可。
</ParamField>

<ParamField body="resolution" type="string" default="standard">
  视频分辨率（仅 `sora-2-pro-preview` 支持）
...
<ParamField body="image_urls" type="array<url>">
  ...
  **重要限制**：只取第一张图片作为参考图
  ...
  当使用参考图片时，API 会**自动检测图片的宽高比，并强制覆盖**用户提供的 `aspect_ratio` 参数。

  **解决方案**：
  - 如需精确控制输出视频的宽高比，**请勿使用参考图片**（纯文生视频）
  - 如必须使用参考图片，请确保参考图片本身是目标宽高比

  示例：`["https://example.com/reference-image.jpg"]`
...
- **计费方式**：按视频时长（秒）计费
- **具体价格**：请参考 [定价页面](https://apimart.ai/model/sora-2)
