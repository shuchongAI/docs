...
<ParamField body="prompt" type="string" required>
  ...
  **指定音色语法**：在 prompt 中使用 `<<<voice_1>>>` 来引用 `voice_list` 中的音色，序号对应 `voice_list` 数组的排列顺序（从 1 开始）。语法结构越简单越好。

  示例：`"男人<<<voice_1>>>说："你好""`

  <Warning>
  - 使用 `<<<voice_1>>>` 引用音色时，`audio` 参数必须为 `true`
  - 当 `voice_list` 不为空且 prompt 中引用了音色时，按**"有指定音色"**计量计费
  </Warning>
</ParamField>
...
<ParamField body="image_urls" type="array<url>">
  ...
  - **尾帧与音频互斥**：在 `pro` 模式下，尾帧（传 2 张图片）和音频（`audio: true`）不能同时使用
...
<ParamField body="voice_list" type="array">
  ...
  在 `prompt` 中通过 `<<<voice_N>>>` 引用对应音色：

  ```json
  {
    "prompt": "男人<<<voice_1>>>说："你好"，女人<<<voice_2>>>说："你好呀"",
    "audio": true,
    "voice_list": [
  ...
  - 一次视频生成任务最多引用 **2 个**音色
  - 在 prompt 中引用音色时，`audio` 必须为 `true`
  - 当 `voice_list` 不为空且 prompt 中引用了音色时，按**"有指定音色"**计量计费
  </Warning>
</ParamField>
