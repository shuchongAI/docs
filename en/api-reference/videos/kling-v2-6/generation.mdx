...
<ParamField body="prompt" type="string" required>
  ...
  **Voice reference syntax**: Use `<<<voice_1>>>` in the prompt to reference voices from the `voice_list` array. The number corresponds to the order in the `voice_list` array (starting from 1). Keep the syntax structure as simple as possible.

  Example: `"A man<<<voice_1>>> says: \"Hello\""`

  <Warning>
  - When using `<<<voice_1>>>` to reference voices, the `audio` parameter must be `true`
  - When `voice_list` is not empty and the prompt references voices, billing is charged at the **"specified voice"** rate
  </Warning>
</ParamField>
...
<ParamField body="voice_list" type="array">
  ...
  Reference voices in the `prompt` using `<<<voice_N>>>`:

  ```json
  {
    "prompt": "A man<<<voice_1>>> says: \"Hello\", a woman<<<voice_2>>> says: \"Hi there\"",
    "audio": true,
    "voice_list": [
  ...
  - Maximum **2** voices per video generation task
  - When referencing voices in the prompt, `audio` must be `true`
  - When `voice_list` is not empty and the prompt references voices, billing is charged at the **"specified voice"** rate
  </Warning>
</ParamField>
...
{
  "model": "kling-v2-6",
  "prompt": "A news anchor<<<voice_1>>> speaking to the camera",
