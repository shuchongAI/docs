...
3. **APIMart API key obtained**  
   Log in to the [APIMart Console](https://api.apimart.ai/console/token) to get your API key (starts with `sk-`)

<Note>
**Tip:** If you don't have an APIMart account yet, please register at [APIMart](https://api.apimart.ai) first and obtain an API key.
</Note>
...
<Note>
**Configuration Notes:**
- **API Key**: Must be the key starting with `sk-` obtained from [APIMart Console](https://api.apimart.ai/console/token)
...
3. **Verify key validity**:
   - Check key status at [APIMart Console](https://api.apimart.ai/console/token)
...
4. **Monitor usage**:
   - Regularly check the [APIMart Console](https://api.apimart.ai/console)
...
<Card title="Get Started with APIMart" icon="rocket" href="https://api.apimart.ai">
