---
title: "Quick Start"
description: "Quickly start using our API services"
---

# Quick Start

Welcome to our API services! This guide helps you quickly get started with image and video generation.

## Step 1: Get an API Key

1. Visit the API Key Management page: https://your-domain.com/api-keys
2. Sign in to your account
3. Create a new API key
4. Save your key securely (it is shown only once)

## Step 2: Choose a Model

We provide multiple AI models to choose from.

### Image Generation Models
- GPT-4o: High‑quality image generation
- Seedream-4.0: Professional image generation
- Nano-Banana: Fast image generation
- Qwen: Chinese‑optimized image generation
- Wan2.5-Image: Latest image generation technology

### Video Generation Models
- VEO3: High‑quality video generation
- Sora2: Professional video generation
- Wan2.5-Video: Latest video generation technology

## Step 3: Send a Request

### Image generation example

```bash
curl -X POST https://apimart.ai/v1/images/generations \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o-image",
    "prompt": "A cute panda",
    "size": "1024x1024",
    "n": 1
  }'
```

### Video generation example

```bash
curl -X POST https://apimart.ai/v1/videos/generations \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "veo3",
    "prompt": "Waves crashing against the shore",
    "duration": 10,
    "resolution": "1920x1080"
  }'
```

## Step 4: Check Task Status

Because we use asynchronous processing, you need to query task status to obtain results.

```bash
curl -X GET https://apimart.ai/v1/tasks/YOUR_TASK_ID \
  -H "Authorization: Bearer YOUR_API_KEY"
```

## What’s Next

<Card title="View API docs" icon="book" href="/en/api-reference/introduction">
  Learn more about all available API endpoints.
  </Card>

<Card title="Development Guide" icon="code" href="/en/development">
  Learn how to integrate the API into your application.
</Card>

