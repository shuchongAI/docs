...
<ParamField body="prompt" type="string" required>
  ...
  **음성 참조 구문**: 프롬프트에서 `<<<voice_1>>>`을 사용하여 `voice_list` 배열의 음성을 참조할 수 있습니다. 번호는 `voice_list` 배열의 순서에 해당합니다 (1부터 시작). 구문 구조는 가능한 한 간단하게 유지하세요.

  예시: `"남자<<<voice_1>>>가 말한다: "안녕하세요""`

  <Warning>
  - `<<<voice_1>>>`로 음성을 참조할 때 `audio` 매개변수는 `true`여야 합니다
  - `voice_list`가 비어있지 않고 프롬프트에서 음성을 참조한 경우 **"지정 음성"** 요금으로 과금됩니다
  </Warning>
</ParamField>
...
<ParamField body="voice_list" type="array">
  ...
  `prompt`에서 `<<<voice_N>>>`을 사용하여 해당 음성 참조:

  ```json
  {
    "prompt": "남자<<<voice_1>>>가 말한다: \"안녕하세요\", 여자<<<voice_2>>>가 말한다: \"안녕!\"",
    "audio": true,
    "voice_list": [
  ...
  - 한 번의 비디오 생성 작업당 최대 **2개** 음성 참조 가능
  - 프롬프트에서 음성을 참조할 때 `audio`는 `true`여야 합니다
  - `voice_list`가 비어있지 않고 프롬프트에서 음성을 참조한 경우 **"지정 음성"** 요금으로 과금됩니다
  </Warning>
</ParamField>
...
{
  "model": "kling-v2-6",
  "prompt": "뉴스 앵커<<<voice_1>>>가 카메라를 향해 뉴스를 전하는 모습",
