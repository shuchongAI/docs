...
2. **APIMart API キー**  
   [APIMart コンソール](https://apimart.ai/console/token)にログインして API キーを取得（`sk-` で始まる）

<Note>
**ヒント：** APIMart アカウントをまだお持ちでない場合は、まず [APIMart](https://apimart.ai) で登録して API キーを取得してください。
</Note>
...
<Note>
**重要な注意事項：**
- `sk-xxxxxxxxxxxx` を [APIMart コンソール](https://apimart.ai/console/token)から取得した実際の API キーに置き換えてください
...
3. **キーの有効性を確認**：
   - [APIMart コンソール](https://apimart.ai/console/token)でキーの状態を確認
   - アカウントに十分な残高があることを確認
