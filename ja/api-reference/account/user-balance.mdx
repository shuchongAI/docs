...
{
  "success": false,
  "message": "ユーザー残高の取得に失敗しました: record not found"
}
```
...
{
  "success": false,
  "message": "使用済み残高の取得に失敗しました: record not found"
}
```
...
<ResponseField name="remain_balance" type="float">
  ユーザー残高（成功時に返却）。`unlimited_quota` が `true` の場合、値は `-1` になります
</ResponseField>
...
| データソース | TokenのRemainQuotaとUsedQuota | Userのquotaとused_quota |
